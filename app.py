...
# ---------------------------------------
# Поддержка как OpenAI, так и DeepSeek API
...
OLLAMA_HOST = os.getenv("OLLAMA_HOST", "https://api.openai.com/v1")
MODEL_NAME = os.getenv("MODEL_NAME", "gpt-3.5-turbo")

# Устанавливаем API ключ в зависимости от выбранного провайдера
...
def get_rag_response(question, chat_history):
    ...
    try:
        ...
        # Если используем DeepSeek напрямую
        if USE_DEEPSEEK and DEEPSEEK_API_KEY:
...
# Создаём интерфейс
with gr.Blocks(title="RAG Support Bot", theme=gr.themes.Soft()) as demo:
    gr.Markdown("# 🤖 RAG Support Bot — панель поддержки")
    ...
    with gr.Tab("⚙️ Настройки"):
        ...
        if USE_DEEPSEEK and DEEPSEEK_API_KEY:
            ...
            gr.Markdown(f"**DeepSeek API Key:** {'✅ Установлен' if DEEPSEEK_API_KEY else '❌ Не установлен'}")
        else:
        ...
        gr.Markdown("""
        **Для DeepSeek API (рекомендуется):**
        1. Зарегистрируйтесь на https://platform.deepseek.com/
        2. Получите бесплатный API ключ
        3. Установите переменную окружения `DEEPSEEK_API_KEY=your-key`
        4. Установите `USE_DEEPSEEK=true`
        
        **Для OpenAI API:**
        1. Установите переменную окружения `OPENAI_API_KEY`
        2. Установите `USE_DEEPSEEK=false`
        
        **Для локальной Ollama:**
        1. Установите `OLLAMA_HOST=http://localhost:11434`
...
if __name__ == "__main__":
    ...
    if USE_DEEPSEEK:
        if not DEEPSEEK_API_KEY:
            ...
            print("Установите переменную окружения DEEPSEEK_API_KEY=your-key")
        else:
